...
trace to a number in the synthesis grounding (the deterministic fact digest plus
the quoted desk snippets). A figure that appears nowhere in the evidence is a
fabrication — the classic "confident wrong number" this app exists to prevent, and
...
the figures that carry compliance weight. One bounce back to synthesis with the
offending figures, then deliver with a warning banner.
...
MAX_ATTEMPTS = 2  # initial synthesis + one bounce

_NUM = re.compile(
    r"(?P<neg>-)?\s*(?P<dollar>\$)?\s*(?P<num>\d[\d,]*(?:\.\d+)?)\s*(?P<pct>%)?"
)
...
def _backed(unit: str, value: float, allowed: list[tuple[str, float]]) -> bool:
    for a_unit, a_value in allowed:
        if a_unit == unit and abs(a_value - value) <= max(0.1, 0.02 * abs(a_value)):
            return True
    return False
...
def audit_answer(answer: str, grounding: str) -> AuditResult:
    ...
    for unit, value in extract_figures(answer):
        if not _backed(unit, value, allowed):
            shown = f"${value:,.2f}" if unit == "$" else f"{value:g}%"
...
def make_audit_node(context: AgentContext) -> Callable[[AgentState], dict]:
    def audit_node(state: AgentState) -> dict:
        ...
        feedback = (
            ...
            "correct them, using only numbers from the digest and desk snippets: "
